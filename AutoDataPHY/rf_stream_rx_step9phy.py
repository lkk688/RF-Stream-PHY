...
# -*- coding: utf-8 -*-
"""
rf_stream_rx_step9phy.py  – Step 9 RX: Per-Symbol Pilot Channel Tracking

Extends step8 with per-symbol pilot-based channel estimation to improve
equalization in frequency-selective wireless channels.

Step8 used:
  • LTF channel estimate H (static over the entire packet)
  • Scalar pilot-phase correction per OFDM symbol

Step9 replaces the scalar phase tracker with full per-symbol H(f):
  • Initial H from LTF (same as step8)
  • Per data symbol: LS channel estimate at 4 pilot subcarriers
    (-21, -7, 7, 21) → cyclic-aware linear interpolation across
    all 64 FFT bins → ZF equalization with per-symbol H(f)
  • No additional scalar phase step — per-symbol H captures both
    channel frequency-selectivity and residual CFO phase drift
  • Per-symbol H array stored in NPZ diag as "H_per_sym" for offline
    channel analysis

All gate / mixture-logging / MT inferencer logic is inherited unchanged
from step8.  This step is the preparation stage for wireless (antenna)
deployment — verify on cable first, then remove the cable.
"""
    ...
    def try_demod_at(rxw: np.ndarray, stf_idx: int):
        ...
        for conj_comp in [False, True]:
            ...
            for si in range(cfg.max_ofdm_syms_probe):
                ...
                # ── Stage 1: bulk LTF equalization (same as step8) ───────────
                Ye = Y.copy()
                ...
                # ── Stage 2: per-symbol residual correction from 4 pilots ────
                # On a flat/static channel H_res ≈ 1.0 → no change vs step8.
                # On a wireless multipath/time-varying channel H_res captures
                # the deviation of the instantaneous H from the LTF estimate.
                H_res = pilot_residual_correction(Ye, pilot_sign, pilot_vals)
                h_per_sym.append(H_res.copy())
                for k in range(-26, 27):
                    if k == 0: continue
                    idx = sc_to_bin(k)
                    if np.abs(H_res[idx]) > 1e-6:
                        Ye[idx] = Ye[idx] / (H_res[idx] + 1e-12)
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    ap = argparse.ArgumentParser("Step9 PHY RX – per-symbol pilot channel tracking")
    ...
    print("\n" + "=" * 70)
    print("Streaming RX  (Step9 PHY – Per-Symbol Pilot Channel Tracking)")
    print("=" * 70)
