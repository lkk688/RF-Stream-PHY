...
# -*- coding: utf-8 -*-
"""
rf_stream_rx_step6phy.py  – Step 7 RX

Builds on step6 and adds:

  • Robust energy gate: MAD z-score (--energy_z_th, default 8.0) replaces
    the fragile maxe < p10*1.1 threshold – far fewer false detections
  • NCC (normalised cross-correlation) per top-K STF candidate
      ncc_i = corr[i] / (stf_ref_e * xseg_e_i)
    Demod gated by --ncc_min (default 0.15) instead of raw corr_norm
  • Hard-negative FAIL NPZ logging (--save_fail_npz) for offline ML /
    Neural Gate training: saved when ncc_best >= fail_save_ncc_min OR
    z >= fail_save_z_min, with optional subsampling (--fail_npz_prob)
  • Expanded NPZ (both OK and FAIL): corr, topk_idx/corr_norm/ncc,
    energy_ds; when available: H, snr_db_sc, evm_per_sym,
    pilot_phase_err, Yeq_data
  • Richer terminal print on OK: z, ncc_best, cfo, snr, evm + payload line
...
def dsp_thread(
    ...
    # CSV – step7 adds med/mad/z and ncc_best/ncc_best_idx
    ...
    def try_demod_at(rxw: np.ndarray, stf_idx: int):
        ...
        for conj_comp in [False, True]:
            ...
            ds_per_sym    = []   # per-symbol equalized data subcarriers
            pilot_ph_list = []   # per-symbol pilot phase corrections
            evm_list      = []
    ...
    try:
        while not stop_ev.is_set():
            ...
            # Per-candidate NCC:  ncc_i = corr[i] / (stf_ref_e * xseg_e_i)
            ...
            if best_ok:
                ...
                print(f"[RX] OK  seq={best_seq}  {payload_len}B  "
                      ...
                      f"evm={best_diag.get('probe_evm', 0):.3f}{ber_str}")
                try:
                ...
                if cfg.save_npz:
                    _save_npz(
                        ...
                        top_idx=top_idx, topk_ncc=topk_ncc, e=e,
                        meta={
            ...
            elif cfg.save_fail_npz and best_diag:
                if ncc_best >= cfg.fail_save_ncc_min or z >= cfg.fail_save_z_min:
                    if cfg.fail_npz_prob >= 1.0 or random.random() < cfg.fail_npz_prob:
                        _save_npz(
...
# ─────────────────────────────────────────────────────────────────────────────
# NPZ helper  (shared by OK and FAIL paths)
# ─────────────────────────────────────────────────────────────────────────────
def _save_npz(
    path: str,
    rxw: np.ndarray,
    ...
    kw = dict(
        ...
        topk_corr_norm=corr_norm[top_idx].astype(np.float32),
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    ap = argparse.ArgumentParser("Step7 PHY RX – robust gate + NCC + FAIL NPZ")
    ...
    ap.add_argument("--energy_z_th", type=float, default=8.0,
                    help="MAD z-score threshold for energy gate (replaces p10*1.1)")
    ...
    ap.add_argument("--xcorr_min_peak", type=float, default=0.2,
                    help="Kept for backward compat; gating now uses --ncc_min")
    ap.add_argument("--ncc_min",        type=float, default=0.15,
                    help="Min NCC for a top-K STF candidate to enter demod")
    ...
    ap.add_argument("--save_fail_npz",    action="store_true",
                    help="Save hard-negative FAIL NPZ for offline ML")
    ...
    ap.add_argument("--fail_npz_prob",     type=float, default=1.0,
                    help="Subsampling probability for FAIL NPZ (0–1)")
    ...
    print(f"  energy_z_th={cfg.energy_z_th}  ncc_min={cfg.ncc_min}"
          f"  save_fail_npz={cfg.save_fail_npz}")
    print("=" * 70)
...
# QPSK BER sweep with step7 gates:
python3 rf_stream_rx_step6phy.py \
  ...
  --probe_syms 80 \
  --rx_gain_sweep "60,55,50,45,40,35,30,25" --gain_step_s 20 \
  --save_npz --save_fail_npz --fail_npz_prob 0.2 \
  --out_root rf_stream/ber_sweep

# QAM16 BER sweep with FAIL NPZ collection:
python3 rf_stream_rx_step6phy.py \
  --uri ip:192.168.2.2 --fc 2.3e9 --fs 3e6 \
