...
# -*- coding: utf-8 -*-
"""
rf_stream_rx_step8phy.py  – Step 8 RX: Mixture Logging + Neural Gate

Extends step7 (rf_stream_rx_step6phy.py) with:

  • GateInferencer  – optional neural gate loaded from gate_model.pt
      predict(corr_norm, topk_ncc, energy_ds, rxw, meta) -> gate_p ∈ [0,1]
      Never blocks decoding; only influences what gets saved.

  • MultiTaskInferencer – drop-in replacement for GateInferencer using the
      multi-task preamble CNN (multitask_v1.pt).  Extracts 800-sample raw
      preamble IQ from rxw at the best NCC offset; additionally logs mod_p
      (QPSK/QAM-16) and snr_pred (dB) as self.last_mod_p / self.last_snr_pred.
      Auto-detected from checkpoint keys at load time.

  • MixtureLogger   – per-window save decision:
      "ok"        – decoded successfully, always saved (save_npz=True)
      "fail_gate" – packet-like (high z or ncc), gate_p >= threshold,
                    subsampled by fail_npz_prob
      "fail_bg"   – random background sample before energy gate,
                    subsampled by bg_save_prob; keeps dataset balanced
      "skip"      – not saved

  • Expanded CSV columns: gate_p, save_tag (added to every row)

  • New CLI args: --gate_model, --gate_threshold, --bg_save_prob

All PHY logic (STF/LTF, channel estimation, OFDM demod, CRC) is
identical to step7 – not changed.
"""
...
def dsp_thread(
    ...
    # CSV – step8 adds gate_p and save_tag
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    ap = argparse.ArgumentParser("Step8 PHY RX – mixture logging + neural gate")
    ...
    # Step 8: neural gate + background saving
    ...
    print("\n" + "=" * 70)
    print("Streaming RX  (Step8 PHY – Mixture Logging + Neural Gate)")
    print("=" * 70)
...
python3 rf_stream_rx_step8phy.py \
